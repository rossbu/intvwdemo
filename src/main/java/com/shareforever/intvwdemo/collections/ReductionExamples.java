...
//        Averager1 averageCollect = roster.stream()
//                .filter(p -> p.getGender() == Person.Sex.MALE)
//                .map(Person::getAge).collect(Averager1::new, Averager1::accept, Averager1::combine);
        // (first, second) -> first.accept(second) same as Averager::accept;
...
//        System.out.println("Average age of male members: " + averageCollect.average());
    ...
    static class Averager1 implements IntConsumer {
        private int total = 0;
        private int count = 0;
        ...
        public void combine(Averager1 other) {
            total += other.total;
            count += other.count;
        }
